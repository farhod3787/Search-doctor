...
const Hospital = require('../moduls/hospitals')
...
router.post('/:token', async function (request, response, next) {
    ...
    if (obj.isAdmin) {
        ...
        const hos = new Hospital(hospital);
        hos.save().then( (res) =>{
...
router.get('/', (request, response, next) =>{
    var hospitals = [];
    Hospital.find().then( (all) =>{
        for (let i=all.length-1; i>=0; i--) {
            hospitals.push(all[i]);
        }
        response.status(200).json(hospitals);
    }).catch(  (err)=>{
        console.log(err)
        response.status(400).json({message: "Error in Get All hospitals"});
...
router.get('/:id', async function(request, response) {
    ...
    await Hospital.findById(id).then( (res)=>{
            if(!res) {
                success = false;
                data = "User not found"
...
router.delete('/:id/:token', async function (request, response, next ){
    ...
    if (obj.isAdmin) {
              Hospital.findByIdAndDelete(id).then( res => {
                  if (!res) {
...
router.patch('/:id', async function(request, response) {
    ...
    let body = request.body;
        Hospital.findByIdAndUpdate(id, { $set: body }, { new: true }).then(res => {
            if (res) {
